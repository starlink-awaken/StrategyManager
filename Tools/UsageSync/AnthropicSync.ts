...
export class AnthropicSync implements UsageSync {
  ...
  async fetchUsage(period?: { start: Date; end: Date }): Promise<UsageData[]> {
    try {
      // 调用 Claude CLI
      // claude 命令的使用量查询（需要根据实际CLI参数调整）
      const { stdout } = await execAsync(
        `ANTHROPIC_API_KEY=${this.apiKey} claude api usage`,
      );
  ...
  private transformData(
    ...
    const models = data.models || [
      { name: "claude-3.5-sonnet", ...data.usage },
    ];
    ...
    return models.map((modelData: any) => ({
      ...
      usage: {
        ...
        totalTokens:
          (modelData.input_tokens || data.usage?.input_tokens || 0) +
