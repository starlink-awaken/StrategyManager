...
export class UsageSyncCLI {
  ...
  constructor() {
    // 配置目录
    this.configDir = path.join(process.env.HOME || '.', '.config', 'strategy-manager');
    this.dataDir = path.join(this.configDir, 'data');
    this.authPath = path.join(process.env.HOME || '.', '.local', 'share', 'opencode', 'auth.json');
  ...
  private registerSyncsFromAuthFile(): void {
    ...
    try {
      ...
      if (auth['github-copilot']?.access || auth['github-models']?.key) {
        try {
          const sync = GitHubSync.fromOpenCodeAuth(auth['github-copilot'] || auth['github-models']);
          this.coordinator.register(sync);
        } catch (e) {
          // Silently skip
        }
    ...
    try {
      ...
      for (const [provider, data] of Object.entries(results)) {
        if (data.status === 'success' && data.data) {
          console.log(`\x1b[32m✓ ${provider.padEnd(20)}\x1b[0m ${data.data.recordCount} records, ${data.data.totalTokens} tokens`);
          successCount++;
        } else if (data.status === 'error') {
          console.log(`\x1b[31m✗ ${provider.padEnd(20)}\x1b[0m ${data.error || 'Unknown error'}\x1b[0m`);
          failureCount++;
        }
      }
      
      console.log('');
      console.log(`\x1b[32m✓ 成功: ${successCount}\x1b[0m  \x1b[31m✗ 失败: ${failureCount}\x1b[0m  \x1b[36m⏱ 耗时: ${duration}s\x1b[0m`);
      console.log('');
      
      // 保存数据
      const dataPath = path.join(this.dataDir, `sync-${new Date().toISOString().split('T')[0]}.json`);
   ...
   * 报告命令 - 生成成本报告
   */
  private async handleReport(format?: string, options?: string[]): Promise<void> {
    ...
    const output = isJson ? report.generateJsonReport() : report.generateTextReport();
    console.log(output);
    
    // 可选：保存到文件
    if (options?.includes('--save')) {
      const ext = isJson ? 'json' : 'txt';
      const reportPath = path.join(this.dataDir, `report-${new Date().toISOString().split('T')[0]}.${ext}`);
    ...
    if (fs.existsSync(authPath)) {
      try {
        ...
        for (const [service, info] of Object.entries(auth)) {
          ...
          const status = serviceInfo.access || serviceInfo.key ? '\x1b[32m✓\x1b[0m' : '\x1b[31m✗\x1b[0m';
  ...
  private async validateConfig(): Promise<void> {
    ...
    try {
      ...
      let validCount = 0;
      const requiredServices = ['anthropic', 'openai', 'github-copilot', 'google'];
      
      for (const service of requiredServices) {
        if (auth[service]?.access || auth[service]?.key || auth[service]?.refresh) {
      ...
      console.log('');
      console.log(`\x1b[32m✓ 验证完成: ${validCount}/${requiredServices.length} 必需服务已配置\x1b[0m`);
    } catch (e) {
      throw new Error(`无法解析认证文件: ${e instanceof Error ? e.message : String(e)}`);
    }
  }
  ...
  private async handleHealth(): Promise<void> {
    ...
    const providers = ['anthropic', 'openai', 'zhipu', 'github', 'gemini', 'deepseek', 'silicon-flow'];
    ...
    for (const provider of providers) {
      try {
        ...
        console.log(`\x1b[31m✗ ${provider.padEnd(20)}\x1b[0m ${(e as any).message || 'Unknown error'}\x1b[0m`);
      }
    }
