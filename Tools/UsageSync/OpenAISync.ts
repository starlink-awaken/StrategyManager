...
export class OpenAISync implements UsageSync {
  ...
  async fetchUsage(period?: { start: Date; end: Date }): Promise<UsageData[]> {
    try {
      ...
      const response = await fetch(`${this.baseURL}/usage?date=${this.formatDate(startDate)}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${this.apiKey}`,
  ...
  private transformData(data: any, period: { start: Date; end: Date }): UsageData[] {
    ...
    for (const item of data.data || []) {
      ...
      // 累加使用量
      const contextTokens = item.n_context_tokens_total || item.prompt_tokens || 0;
      const generatedTokens = item.n_generated_tokens_total || item.completion_tokens || 0;
