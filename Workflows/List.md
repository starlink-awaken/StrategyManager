...
1. 调用 listStrategies() 或 listStrategiesWithOptions() 以获取所有可用策略的元信息。

- 函数签名： listStrategies(): StrategyMetadata[]
- 函数签名： listStrategiesWithOptions({ includeDynamic?: boolean }): StrategyMetadata[]
- 位置： skills/StrategyManager/Tools/ManageStrategies.ts

2. 若返回数组为空，报告错误（没有找到可用的策略）并退出。
3. 否则，准备表格显示字段：名称、成本级别、描述（截断为 30 字符）、来源、状态（高亮当前策略）。
4. 使用 displayStrategies(includeDynamic) 输出格式化表格到控制台。

- 函数签名： displayStrategies(includeDynamic?: boolean): void
...
- displayStrategies(includeDynamic?: boolean):
  - 调用 listStrategies() 获取策略数组。
  - 若空则输出错误信息（"没有找到可用的策略"）。
  - 构建表头 ['名称','成本级别','描述','来源','状态']，并为每条策略生成一行数据。
